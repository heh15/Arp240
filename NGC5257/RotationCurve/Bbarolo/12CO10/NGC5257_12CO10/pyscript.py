...
f0 = fits.open(outfolder+'/maps/_0mom.fits') 
f1 = fits.open(outfolder+'/maps/_1mom.fits') 
f2 = fits.open(outfolder+'/maps/_2mom.fits') 
...
image = fits.open('../NGC5257_12CO10_combine_smooth_sub.fits') 
...
for k in range (len(files_mod)): 
	...
	for j in range (0,3): 
		for i in range (0,5): 
			...
			velo_kms = (chan+1--39)*10.0019+-599.4
			...
			if (j==i==0): 
				...
				bmaj, bmin, bpa = 7.28667/float(xmax-xmin), 6.32/float(ymax-ymin),-87.314
...
xminpv, xmaxpv = np.floor(crpixpv-1-52), np.ceil(crpixpv-1 +52) 
...
zmin_wcs, zmax_wcs = -199.933, 300.15
...
labsize = 15 
palab = ['$\phi = $109$^\circ$', '$\phi = $199$^\circ$'] 

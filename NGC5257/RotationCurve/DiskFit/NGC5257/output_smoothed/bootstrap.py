...
infile=[];outfile=[]
for i in range(n):
    infile.append('velsf_12CO10_smooth_'+str(i)+'.inp')
    outfile.append('NGC5257_12CO10_vel_smooth_'+str(i)+'.out')

origfile='velsf_12CO10_smooth.inp'
...
for i in range(n):
    if i !=0:
        ...
        with open(infile[i],'w') as output:
            output.write(content)
        call(['find', '.','-name','NGC5257_12CO10*','-and','!','-name','NGC5257_12CO10*.out','-delete'])
# run the file

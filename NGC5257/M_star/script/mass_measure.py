...
Image='spitzer_45um_regrid.fits'
wcs_cut=fits_import(Image)[0]
data_cut=fits_import(Image)[1]
shape=data_cut.shape
flux=np.empty(shape=(0,0))
...
output=np.transpose(np.vstack([radius_kpc,mass_ring,error_ring]))
...
m_corr=np.copy(m_star)
m_corr[1][0]=m_corr[1][0]/0.6;m_corr[2][0]=m_corr[2][0]/0.6
...
fraction1=np.divide(m_gas[1],m_corr[1]+m_gas[1])
error1=np.divide(m_gas[2],m_corr[1]+m_gas[1])
...
fig=plt.figure()
arp240=plt.plot(mstar[0],fraction,label='arp240')
N3198=plt.plot(N3198_vel_star[0],fraction1,label='N3198')
# plt.plot(N3198_vel_star[0],N3198_mass_star)
plt.xlabel('kpc')
plt.ylabel('H2/star fraction')
plt.legend()
plt.savefig('comparison.png')

fig=plt.figure()
plt.errorbar(radius_kpc,fraction,error,label='extinction uncorrected')
plt.errorbar(radius_kpc,fraction1,error1,label='extinction corrected')
plt.xlabel('radius (kpc)')
plt.ylabel('molecular gas fraction')
plt.savefig(pictureDir+'fraction_corr.png')

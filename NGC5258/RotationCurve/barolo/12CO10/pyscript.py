...
f0 = fits.open(outfolder+'/maps/NGC5258_0mom.fits') 
f1 = fits.open(outfolder+'/maps/NGC5258_1mom.fits') 
f2 = fits.open(outfolder+'/maps/NGC5258_2mom.fits') 
...
image = fits.open('../Image/NGC5258_12CO10_combine_contsub.fits') 
	...
	for j in range (0,3): 
		for i in range (0,5): 
			...
			if (j==i==0): 
				...
				bmaj, bmin, bpa = 6.7/float(xmax-xmin), 5.35333/float(ymax-ymin),-69.675
...
xminpv, xmaxpv = np.floor(crpixpv-1-60), np.ceil(crpixpv-1 +60) 
...
labsize = 15 
palab = ['$\phi = $206$^\circ$', '$\phi = $296$^\circ$'] 

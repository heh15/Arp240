...
err1_l[4], err1_h[4] = np.genfromtxt(file1,skip_header=1,usecols=(17,18),unpack=True) 
err1_l[5], err1_h[5] = np.genfromtxt(file1,skip_header=1,usecols=(19,20),unpack=True) 
...
f0 = fits.open(outfolder+'/maps/NGC_5258_0mom.fits') 
f1 = fits.open(outfolder+'/maps/NGC_5258_1mom.fits') 
f2 = fits.open(outfolder+'/maps/NGC_5258_2mom.fits') 
...
image = fits.open('image/NGC5258_12CO21_pbcor_cube.fits') 
	...
	for j in range (0,3): 
		for i in range (0,5): 
			...
			if (j==i==0): 
				...
				bmaj, bmin, bpa = 10.0804/float(xmax-xmin), 5.25591/float(ymax-ymin),115.342
...
xminpv, xmaxpv = np.floor(crpixpv-1-136), np.ceil(crpixpv-1 +136) 
...
labsize = 15 
palab = ['$\phi = $207$^\circ$', '$\phi = $297$^\circ$'] 
